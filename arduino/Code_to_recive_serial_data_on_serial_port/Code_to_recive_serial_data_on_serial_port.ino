...
const int servoPin3 = 9; //ritsjratsj
...
int drumPins [] = {
  0,1};
unsigned long drumLastHit [] = {
  0, 0, 0, 0};
int delayBetweenHits [] = {400, 400, 800, 400};
int timeToResetServo [] = {100, 100, 100, 100};
int servoEngagePosition [] = {30, 30, 30, 30};
int servoDisengagePosition [] = {0, 0, 0, 0};
boolean drumBusy [] = {
...
void engage(int i) {
  ...
  if (drumBusy[i] == false
    && millis() - drumLastHit[i] > delayBetweenHits[i]) {
    drumBusy[i] = true;
...
void disengageAll() {
  for (int i = 0; i < NUMBER_OF_SERVOS; i++) {
    if (drumBusy[i] == true
      && millis() - drumLastHit[i] > timeToResetServo[i]) {
      drumBusy[i] = false;
      disengage(i);
      digitalWrite(ledPin, LOW);
    }
